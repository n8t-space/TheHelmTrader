...
Three operator-authored SQLite stores live beside `trades.db` at the project root, all **git-ignored** (never push): `trades.db` (recorder-fed), `journal.db` (per-trade journal), `expenses.db` (business expense ledger). Only `trades.db` is recorder-managed/read-only; the other two are read-write via their routers.
