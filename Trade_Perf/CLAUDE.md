...
FastAPI (`dashboard/api/main.py`) on `127.0.0.1:8000` serves both `/api/*` routes AND the built React SPA from `dashboard/web/dist/`. Vite dev server (`run_dev.ps1`) only used for active frontend development. Pages: Home / Trade Performance / Journal / Expenses / Signal Analysis (+ Health / Settings / Support).

Three operator-authored SQLite stores live beside `trades.db` at the project root, all **git-ignored** (never push): `trades.db` (recorder-fed), `journal.db` (per-trade journal), `expenses.db` (business expense ledger). Only `trades.db` is recorder-managed/read-only; the other two are read-write via their routers. (The repo-root `Documentation/` branding folder is also git-ignored as of v2.1.1.)

The SPA catch-all (`main.py spa_or_static`) serves `index.html` with `Cache-Control: no-cache` (v2.1.0) so a new build's hash-named bundle always loads -- hashed `assets/` stay cacheable. Without it, browsers heuristic-cache the HTML and serve a stale UI after an update.
...
Settings is the source of truth for which NT accounts are visible site-wide. The Settings Accounts tab renders one row per account known to trades.db (plus any pre-seeded sims), with radios: `Hidden | Live | Eval | PA | Sim`. The union of the four non-hidden buckets = the visible set. NT-default sims (`Sim101`, `Playback101`, `Backtest`, `SimBetaSIM`) ship pre-listed under Simulation.

**PA = Paid Account (v2.0.1).** `accounts.paid` is a first-class bucket (a passed eval -> funded), sibling to `live`: included in `visible_accounts()`, `home._account_category` + the Home cumulative-earnings card (own "Paid (PA)" line), the FilterBar quick-groups, and the Strategy-tab per-account config set (Live + Eval + **PA**). Move an eval to PA once it passes; its `account_configs` entry follows it by account id.

**Entity tagging (v2.0.1).** `accounts.entities[id]` = `"personal" | "llc"` (unset -> personal); `settings.llc_name` is the display label (blank -> "LLC"). Per-account Entity selector + the entity-name field live on the Accounts tab. Drives the Personal/LLC split on the Expenses page. Sizing/visibility are unaffected by entity.

**Accounts-tab numeric columns (v2.0.1).** Profit target (Eval rows only -> `account_configs.profit_target`, the amount to pass the eval) and Trailing DD (Live/Eval/PA -> the EXISTING `account_configs.trailing_dd_limit`, one source of truth, also on the Strategy tab). Both write `account_configs[id]`, so the entry persists across bucket moves.
...
- **Eval/Sim fills book $0 commission + $0 fee in NT8 (load-bearing).** So the Helm's net P&L on eval accounts is effectively GROSS unless a per-instrument commission rate is set in Settings -> Accounts -> Commissions. When a prop dashboard (e.g. Tradeify) shows LESS profit than the Helm, the gap is almost always the firm's commissions/fees the Helm hasn't been told about. Fix: set the symbol's all-in per-side cost (commission is `$/contract, per side`); it flows through `derive_trades` into the Eval Progress card, Trade Performance, and tax. Verified case (2026-06-25): EVAL 35 Helm $710 vs Tradeify $589.04, reconciled at $2.88/side.
- **Kill switch + watchdog (v2.0.1).** `runtime/watchdog.ps1` honors `~/.helm/kill-switch.json`: pins to the NT PID+StartTime live at kill, keeps the dashboard down until that instance is gone (NT restart) and clears the sentinel on its own startup (service restart resumes). The running watchdog only picks up `watchdog.ps1` edits on `Restart-Service HelmDashboardWatchdog` (elevated; non-elevated fails Access denied). NSSM stop-method timeouts were zeroed + `AppThrottle` set to 1500 on the box so restarts are ~2-3s (registry-only change, not in the repo; re-apply via `nssm set` if the service is reinstalled).
...
- `home.py` — today's session card, action queue, cumulative-earnings by Live / Evals / **Paid (PA)** / Simulation / Signals bucket, and (v2.0.1) `session_calendar` (per-trading-day net realized P&L for the Home month calendar). Buckets read from the live Settings doc.
- `journal.py` (v2.0.1) — per-trade journal CRUD over `journal.db`, keyed on the trade `{first_fill_id}-{last_fill_id}`. `GET/PUT/DELETE /api/journal/{trade_key}` + `GET /api/journal` (list) + `GET /api/journal/entry-screenshots` (declared BEFORE `/{trade_key}` so the literal wins; maps trade_key -> auto-entry screenshot via `fill_linker`, degrades to `{}` not 500). Snapshot stores symbol/account/direction/net P&L/ATM/entry+exit price at journaling time.
- `expenses.py` (v2.0.1) — business expense ledger CRUD over `expenses.db`. `GET/POST/PUT/DELETE /api/expenses`. Rows: date, category (Schedule-C-flavored set), amount, entity (personal|llc), vendor, optional account link, recurring flag (manual), deductible, note. List returns roll-ups (by_category/by_entity/by_account/by_year) + year/entity filters. NOT fed into Home totals (kept separate by design).
- `compliance.py` (v2.0.1) — `GET /api/microscalp-compliance` (route in `main.py`): per-account sub-10s trade% + gross-profit% vs the 50% prop-eval cap, PASS/BREACH. Eval-accounts flagged from `settings.accounts.evals` (NOT extended to PA). Profit ratio uses gross profit (winning trades only).
- `control.py` (v2.0.1) — kill switch. `POST /api/control/kill` writes `~/.helm/kill-switch.json`; the watchdog stops uvicorn within one poll and keeps it down while the NT instance live at kill time persists, lifting on NT restart OR watchdog startup. `GET` status, `DELETE` resume. Read the sentinel with `encoding="utf-8-sig"` (watchdog may BOM it).
- `main.py` also hosts `GET /api/eval-progress` (v2.0.1): per-eval profit-target progress = `profit_target - net realized P&L`, counted since `cash_basis_ts` when set else all-time (reuses the same lexical UTC-Z exit_time compare as `auto_trader._realized_since`).
- `version.py` (v2.0.1) — now surfaces `current_version` / `latest_version` read from the `VERSION` file at HEAD vs origin/main (semver display); SHA fields retained for the updater + dismiss logic.
