...
- `news.py` — `/api/news/today` (GET) + `/api/news/refresh` (POST). Economic-calendar widget for the Home page. Two sources merged + deduped + filtered by impact/currency: **ForexFactory** via the public XML feed at `nfs.faireconomy.media/ff_calendar_thisweek.xml` (no AI required), and **Econoday** via the rendered page scraped at `us.econoday.com/byweek?cust=us&lid=0` and AI-extracted using whichever provider is configured in `ai_backend`. Background refresh loop every `news.refresh_interval_minutes`. Cache lives at `~/.helm/news-cache.json`. Filters are applied at READ time, not refresh, so flipping impact/currency toggles is instant. AI precheck (`_ai_reachable()`) is exposed on `/today` so the Home card can show a "Configure AI to enable Econoday" hint without forcing a real extraction call. **Cert chain caveat:** the FF feed is behind Cloudflare; locked-down corporate boxes (TLS interception, no OCSP egress) may fail with `SSLError(CERTIFICATE_VERIFY_FAILED)`. Workaround: install corp CA into the active `certifi` bundle; the code itself is correct.
- `trading_day.py` — pure helpers (no router). Future-trading-correct date attribution at 5 PM CT roll (CME Globex session start). Consumed by `trades.compute_stats`, `home.py`, `drawdown.py`, and the `_resolve_date_window` helper for the trading_day query params on `/api/trades` + `/api/stats`.
