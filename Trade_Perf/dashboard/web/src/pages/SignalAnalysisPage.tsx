...
function AiConfigDisclaimer() {
  const q = useQuery<{ provider: string; api_key_configured: boolean; configured_model: string | null }>({
    queryKey: ['bot-stats'],
    queryFn: () => fetchJSON('/api/health/bot-stats'),
    staleTime: 60_000,
  })
  const d = q.data
  if (!d) return null
  // Usable = a provider is set up: local Ollama needs a model; a hosted
  // provider (Claude/OpenAI) needs an API key.
  const configured = d.provider === 'ollama' ? !!d.configured_model : d.api_key_configured
  if (!configured) {
    return (
      <div className="card error">
        <strong>AI is not configured.</strong> Signals are generated by an AI provider, and none is set up
        (or its API key is missing). <strong>No new signals will be generated until you configure it</strong> in{' '}
        <Link to="/settings">Settings &rarr; AI Backend</Link>.
      </div>
    )
  }
  return (
    <p className="subtle" style={{ margin: '0 0 12px' }}>
      Signals are AI-generated and require a configured AI provider (currently <strong>{d.provider}</strong>).
      If it is unconfigured or unreachable, no new signals appear &mdash; manage it in{' '}
      <Link to="/settings">Settings &rarr; AI Backend</Link>.
    </p>
  )
}
