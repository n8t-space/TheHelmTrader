...
2. **Journal page issues (user-reported 2026-06-25 EOD):**
   - **Auto-entry screenshots not appearing for filled orders — possibly not even being captured.** INVESTIGATE the whole chain: (a) is `auto_trader.capture_entry_screenshot` toggled ON in Settings? (b) is HelmFeed running + writing `auto_{instrument}_*.png` into `TradingBot/app/data/screenshots/`? (c) the capture only fires on an AUTO-entry fill (`exec.state==filled`, not dry-run) — manual fills never get one; (d) the Journal shows it only if `fill_linker.build_links()` links that signal->trade (`/api/journal/entry-screenshots`). Likely culprits: setting off, HelmFeed not feeding that instrument, or these were manual (not auto-trader) fills so no exec screenshot exists. Confirm with a real Sim auto-fill + check the screenshots dir + `/api/journal/entry-screenshots`.
   - **Make the Journal page sortable** — by Account, Instrument (symbol), tag, P&L, date, discipline, etc. Currently newest-first only; reuse the `lib/sorting` helpers the Trades/Fills tables use.
